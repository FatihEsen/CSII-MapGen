...
import { MapSettings, TerrainType } from '../types';
  ...
  return (
    <div className="p-6 space-y-8">
        ...
        <div className="space-y-5">
           <div>
            ...
            <select 
              ...
              onChange={(e) => setSettings({...settings, terrainType: e.target.value as TerrainType})}
            ...
            >
              <option value="REAL_WORLD">⚡ AWS REAL WORLD (S3 Tiles)</option>
              <option value="PROCEDURAL_COAST">Procedural Coastline</option>
              <option value="RIVER_VALLEY">Deep Valley System</option>
              <option value="RUGGED_HILLS">Mountainous / Hills</option>
              <option value="PLAINS">Open Plains</option>
            </select>
          </div>
        ...
        >
          {isGenerating ? (
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm uppercase">Fetching Terrain...</span>
            </div>
          ) : (
            <>
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
              ...
              <span className="text-sm uppercase tracking-wide">Generate Heightmap</span>
            </>
          )}
        </button>
        ...
        <section className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4">
          ...
          <ul className="space-y-2 text-[10px] text-slate-400">
            <li className="flex items-start">
              ...
              Navigate the map to your desired build location.
            </li>
            <li className="flex items-start">
              <span className="text-blue-500 font-bold mr-2">2.</span>
              Adjust "Max Elevation" to match the terrain peaks.
            </li>
            <li className="flex items-start">
              <span className="text-blue-500 font-bold mr-2">3.</span>
              Click "Generate" and export the 16-bit PNG file.
            </li>
            <li className="flex items-start">
              <span className="text-blue-500 font-bold mr-2">4.</span>
              In CS2 Map Editor, import the PNG using the same Max Elevation.
            </li>
          </ul>
        </section>
      </div>
      ...
      <footer className="pt-8 flex flex-col items-center space-y-1">
        <p className="text-[9px] text-slate-600 font-medium">CS2 Heightmap Generator v4.2</p>
